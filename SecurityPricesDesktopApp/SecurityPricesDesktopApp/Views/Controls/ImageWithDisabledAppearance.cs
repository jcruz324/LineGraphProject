...
namespace SecurityPricesDesktopApp.Views.Controls
{ /// <summary>
  /// Class used to have an image that is able to be gray when the control is not enabled.
  /// Based on the version by Thomas LEBRUN (http://blogs.developpeur.org/tom)
  /// </summary>
    public class ImageWithDisabledAppearance : Image
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ImageWithDisabledAppearance"/> class.
        /// </summary>
        static ImageWithDisabledAppearance()
        {
            // Override the metadata of the IsEnabled and Source property.
            IsEnabledProperty.OverrideMetadata(typeof(ImageWithDisabledAppearance), new FrameworkPropertyMetadata(true, new PropertyChangedCallback(OnAutoGreyScaleImageIsEnabledPropertyChanged)));
            SourceProperty.OverrideMetadata(typeof(ImageWithDisabledAppearance), new FrameworkPropertyMetadata(null, new PropertyChangedCallback(OnAutoGreyScaleImageSourcePropertyChanged)));
        }

        protected static ImageWithDisabledAppearance GetImageWithSource(DependencyObject source)
        {
            var image = source as ImageWithDisabledAppearance;
            if (image == null)
                return null;
        ...
        {
            ImageWithDisabledAppearance image = GetImageWithSource(source);
            if (image != null)
        ...
        protected static void ApplyGreyScaleImage(ImageWithDisabledAppearance autoGreyScaleImg, Boolean isEnabled)
        {
            try
            {
                if (!isEnabled)
